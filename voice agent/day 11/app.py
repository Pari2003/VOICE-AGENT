...
app = FastAPI(title="Voice Agent Day 11 - Robust Error Handling", version="11.0.0")
...
async def home(request: Request):
    ...
    try:
        ...
        return HTMLResponse(
            content=f"<h1>Voice Agent Day 11</h1><p>Error loading page: {str(e)}</p>",
            status_code=500
        )
...
async def list_sessions():
    ...
    try:
        ...
        log_error("session_list_error", str(e))
        raise HTTPException(status_code=500, detail="Error retrieving sessions")

@app.get("/sessions/{session_id}")
async def get_session(session_id: str = Path(..., description="The session ID")):
    """Get session details with error handling"""
    try:
        ...
        log_error("session_get_error", str(e), session_id)
        raise HTTPException(status_code=500, detail="Error retrieving session")

@app.delete("/sessions/{session_id}")
async def clear_session(session_id: str = Path(..., description="The session ID")):
    ...
    try:
        ...
        log_error("session_clear_error", str(e), session_id)
...
async def agent_chat(
    ...
    max_tokens: int = Form(800, description="Maximum tokens for LLM response"),
    ...
    - Robust transcription with retries
    ...
    try:
        ...
        response_data = {
            "success": True,
            "message": "Conversational AI chat completed successfully",
            "session_id": session_id,
            "user_message": user_message,
            "ai_response": ai_response,
        ...
        if audio_result.get("audio_url"):
            ...
            response_data["is_chunked"] = False
        ...
        log_error("agent_chat_error", error_msg, session_id, {"traceback": traceback.format_exc()})
...
# Additional robust endpoints for other functionalities
@app.post("/tts")
async def text_to_speech(data: TextInput):
    ...
    try:
        ...
        audio_result = await safe_generate_audio(data.text, data.voice_id)
        
        if audio_result.get("audio_url"):
            return {"audio_url": audio_result["audio_url"]}
        else:
            return {
                "audio_url": None,
        ...
        raise HTTPException(status_code=500, detail=FALLBACK_MESSAGES["tts_error"])
